...
class rgb_to_ycbcr_jpeg(nn.Module): #RGB B3HW -> YCbCr BHW3
...
class chroma_subsampling(nn.Module): #YCbCr下采样 BHW3 -> BHW,BHW,BHW (Y4,Cb2,Cr2)
    ...
    def forward(self, image):
        ...
        cb = avg_pool(image_2[:, 1, :, :].unsqueeze(1)) #[]出去，少一维->unsqueeze加一维->permute，并squeeze减去一维，得BHW
...
class compress_jpeg(nn.Module): #in: (B,3,H,W) -> out: 3(B,H*W/64,8,8)
